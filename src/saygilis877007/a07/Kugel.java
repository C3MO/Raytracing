...
public class Kugel implements Shape {
    ...
    public Hit intersect(Ray r) {                              //Hit intersect den Ray
        ...
        double b = Vec3.dotProduct(Vec3.multiply(2, x0), d);            //Richtungsvektor, Ortsvektor
        double c = Vec3.dotProduct(x0, x0) - Math.pow(rad, 2);

        double dis = (Math.pow(b, 2) - (4 * a * c));

        double t1 = (-b + Math.sqrt(dis)) / (2 * a);
        double t2 = (-b - Math.sqrt(dis)) / (2 * a);                        //Sind vorgegeben gewesen
        ...
        Vec3 schnittT1Norm = Vec3.normalize(Vec3.divide(Vec3.subtract(schnittT1, center), rad));        //Richtungsvektor, Ortsvektor
        Vec3 schnittT2Norm = Vec3.normalize(Vec3.divide(Vec3.subtract(schnittT2, center), rad));         //Die schnittpunkte werden normalisiert

        if (dis == 0)
            return new Hit(t1, schnittT1, schnittT1Norm,material);

        if (dis > 0) {
            if (t1 > 0 && t1 < t2 || t1 > 0 && t2 < 0)
                return new Hit(t1, schnittT1, schnittT1Norm,material);

            else if (t2 > 0 && t2 < t1 || t2 > 0 && t1 < 0)
                return new Hit(t2, schnittT2, schnittT2Norm,material);
        }
                                                                                //keine, eine oder zwei Lösungen
        return null;                                                            //wenn es unter 0 ist gebe keine Strahl zurück
